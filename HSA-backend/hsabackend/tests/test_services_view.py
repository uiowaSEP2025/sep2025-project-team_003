...
class ServiceViewTest(APITestCase):
    ...
    #     mock_user = Mock(spec=User)
    #     mock_user.is_authenticated = True
    #     o_mock = Mock(spec=Organization)
    #     o_mock.pk = 1
    #     org.return_value = o_mock
    ...
    @patch('hsabackend.views.services.Service.objects.filter')
    @patch('hsabackend.views.services.Organization.objects.get')
    def test_delete_success(self, org, service):
        mock_user = Mock(spec=User)
        mock_user.is_authenticated = True
        o_mock = Mock(spec=Organization)
        o_mock.pk = 1
        ...
        mock_service_qs = MagicMock()
        service.return_value = mock_service_qs
        mock_service_qs.exists.return_value = True
        mock_service_qs.__getitem__.side_effect = lambda x: service_model
        ...
        response = delete_service(request,1)
        assert response.status_code == status.HTTP_200_OK
        service_model.delete.assert_called_once()
