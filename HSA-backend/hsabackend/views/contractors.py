...
def create_contractor(request):
    ...
    serializer = ContractorSerializer(data=contractor_data)

    try:
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response({"message": "Contractor created successfully", "data": serializer.data}, status=status.HTTP_201_CREATED)
...
def edit_contractor(request, id):
    ...
    try:
        serializer.is_valid(raise_exception=True)  # Validate the model instance
        serializer.update(contractor, contractor_data)
